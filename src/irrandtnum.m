...
XTrain = [dataTrainStandardized(1:end-1);tnumTrainStandardized(1:end - 1)];
...
XTest = [dataTestStandardized(1:end-1);tnumTestStandardized(1:end - 1)];
YTest = dataTestStandardized(2:end);

net = predictAndUpdateState(net,XTrain);
[net,YPred] = predictAndUpdateState(net,[dataTrainStandardized(end);tnumTrainStandardized(end)]);
numTimeStepsTest = size(XTest,2);
for i = 2:numTimeStepsTest
    [net,YPred(:,i)] = predictAndUpdateState(net,[YPred(:,i-1);XTest(2,i-1)],'ExecutionEnvironment','cpu');
end
